...
class Rememberer(Handler):
    MODE_ID = {"osu": 1, "taiko": 2, "catch": 3, "mania": 4}

    async def post_event(self, session, e, hook, embed):
        print("Posting to:", hook)
        r = await session.post(
            hook, json={"content": e.event_source_url, "embeds": [embed]}
        )
        if r.status != 204:
            if (await r.json())["code"] in (50027, 10015):
                ...
                await connection.query(
                    [
                        "UPDATE hooks SET status='Cannot send hook, possibly gone -- Won't send anymore.' WHERE hook_url=?",
                        [hook],
                    ]
                )
        elif r.status == 429:
    ...
    async def on_map_event(self, e):
        ...
        async with aiofiles.open("last.txt", mode="w") as f:
            await f.write(
                (e.time + timedelta(seconds=1)).strftime("%Y-%m-%dT%H:%M:%S+00:00")
            )

        print("Generating embed")
        embed = await helper.gen_embed(e, self.app)
        targets = []
        ...
        for hook in hooks:
            ...
            for mode in e.gamemodes:
                if (
                    mode_mapping[mode] in hook_cls.mode
                    and status_mapping[e.event_type] in hook_cls.push_status
                    and not hook_cls.status
                ):
...
def main():
    try:
        with open("last.txt", "r") as f:
            dt = datetime.strptime(f.read().strip(), "%Y-%m-%dT%H:%M:%S+00:00")
    except:
    ...
    ph = Phillip(config.osu_token, dt, handlers=handlers)
    ph.disable_user = True
    ph.run()
